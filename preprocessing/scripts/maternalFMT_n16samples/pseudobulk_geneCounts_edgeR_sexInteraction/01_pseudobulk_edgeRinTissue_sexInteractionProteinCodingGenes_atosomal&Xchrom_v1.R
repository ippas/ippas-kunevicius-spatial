...
# ==============================================================================
# 02_pseudobulk_inTissue_edgeR_sexInteraction_proteinCodingGenes_autosomalAndX_Ensembl115.R
#
# PURPOSE
...
#   6. Retain only protein-coding genes located on autosomes 1-19 or chromosome X.
#      Chromosome Y, MT and non-canonical sequences are excluded.
...
# Keep this analysis separate from the previous analysis of all
...
# The updated annotation RDS must contain the chromosome column.
...
analysis_chromosomes <- c(
  as.character(1:19),
  "X"
)

reference_annotation_label <- paste0(
  "Mus musculus protein-coding genes on chromosomes 1-19 and X from official ",
  "Ensembl ",
  ensembl_release,
  " ",
...
if (!file.exists(ensembl115_protein_coding_rds)) {
  stop(
    ...
    "\nRun download_Ensembl115_mouse_proteinCodingGenes_fromGTF_UPDATED.R first."
  )
}
...
message("Analysis chromosomes: ", paste(analysis_chromosomes, collapse = ", "))
...
#
# The complete local annotation is validated first. The analysis annotation is
# then restricted to autosomes 1-19 and chromosome X.
# ==============================================================================
...
if (!identical(
  as.character(
    ensembl115_annotation_metadata$retained_feature_type[[1]]
  ),
  "gene"
)) {
...
if (!identical(
  as.character(
    ensembl115_annotation_metadata$retained_gene_biotype[[1]]
  ),
  "protein_coding"
)) {
...
ensembl115_protein_coding_annotation_all <-
  ...
  transmute(
    chromosome =
      trimws(as.character(chromosome)),
    ensembl_gene_id = sub(
      "\\.[0-9]+$",
      "",
    ...
    gene_biotype =
      trimws(as.character(gene_biotype))
  ) |>
  filter(
    !is.na(chromosome),
    chromosome != "",
...
ensembl115_protein_coding_annotation_autosomal_and_X <-
  ensembl115_protein_coding_annotation_all |>
  filter(
    chromosome %in% analysis_chromosomes
  ) |>
  mutate(
    chromosome = factor(
      chromosome,
      levels = analysis_chromosomes
    )
  ) |>
  arrange(
    chromosome,
...
if (
  ...
  stop(
    "No protein-coding genes on chromosomes 1-19 or X were found ",
    "in the local Ensembl 115 annotation."
  )
}

unexpected_analysis_chromosomes <- setdiff(
  unique(
    ensembl115_protein_coding_annotation_autosomal_and_X$chromosome
  ),
  analysis_chromosomes
)

if (length(unexpected_analysis_chromosomes) > 0L) {
  stop(
    "Unexpected chromosomes remain in the analysis annotation: ",
    paste(unexpected_analysis_chromosomes, collapse = ", ")
  )
}

if (any(
  ensembl115_protein_coding_annotation_autosomal_and_X$chromosome == "Y"
)) {
  stop("Chromosome Y genes were not removed from the analysis annotation.")
}
...
reference_annotation_info <- tibble::tibble(
  ...
  genes_in_complete_local_annotation =
    nrow(ensembl115_protein_coding_annotation_all),
  genes_in_autosomal_and_X_annotation =
    nrow(
      ensembl115_protein_coding_annotation_autosomal_and_X
    ),
...
# ==============================================================================
# 17. Filter the pseudobulk matrix to Ensembl 115 protein-coding genes on
#     autosomes 1-19 and chromosome X
#
...
protein_coding_filter_status <- tibble::tibble(
  ...
  mutate(
    ...
    protein_coding_filter_status = case_when(
      passed_protein_coding_filter ~
        paste0(
          "passed_ensembl115_protein_coding_",
          "autosomal_and_X_filter"
        ),
      found_in_ensembl115_protein_coding_annotation &
        !located_on_autosomal_or_X_chromosome ~
        paste0(
          "filtered_out_protein_coding_gene_",
          "outside_autosomal_and_X_chromosomes"
        ),
      TRUE ~
        paste0(
          "filtered_out_not_in_ensembl115_",
...
if (length(protein_coding_gene_ids) == 0L) {
  stop(
    "No genes matched the Ensembl 115 protein-coding annotation ",
    "for chromosomes 1-19 and X."
  )
}
...
# Convenient alias used throughout the unchanged edgeR analysis below.
...
if (!identical(
  ...
  stop(
    "Autosomal-and-X protein-coding count matrix and annotation order ",
    "do not match."
  )
}

if (any(
  protein_coding_gene_annotation$chromosome == "Y"
)) {
  stop("Chromosome Y genes remain in the matrix used for edgeR.")
}

message(
  "Genes before the Ensembl 115 protein-coding chromosome filter: ",
  format(
    nrow(
      gene_counts_per_sample_raw_in_tissue_allGenes
    ),
...
message(
  "Protein-coding genes on chromosomes 1-19 and X retained: ",
  format(
    nrow(
      gene_counts_per_sample_raw_in_tissue_proteinCoding
    ),
...
message(
  "Genes removed by the protein-coding chromosome filter: ",
  format(
    nrow(
      gene_counts_per_sample_raw_in_tissue_allGenes
    ) -
    ...
    final_filter_status =
      dplyr::case_when(
        found_in_ensembl115_protein_coding_annotation &
          !located_on_autosomal_or_X_chromosome ~
          paste0(
            "removed_protein_coding_gene_outside_",
            "autosomal_and_X_chromosomes"
          ),
        !found_in_ensembl115_protein_coding_annotation ~
          paste0(
...
genes_in_complete_local_protein_coding_annotation <- nrow(
  ensembl115_protein_coding_annotation_all
)

genes_in_local_autosomal_and_X_annotation <- nrow(
  ensembl115_protein_coding_annotation_autosomal_and_X
)
...
protein_coding_genes_removed_outside_autosomal_and_X <- sum(
  protein_coding_filter_status$
    found_in_ensembl115_protein_coding_annotation &
    !protein_coding_filter_status$
      located_on_autosomal_or_X_chromosome,
...
filtering_summary <- tibble::tibble(
  metric = c(
    ...
    "genes_in_complete_local_Ensembl115_protein_coding_annotation",
    "genes_in_local_Ensembl115_protein_coding_autosomal_and_X_annotation",
    "genes_retained_after_protein_coding_autosomal_and_X_mapping",
    "genes_removed_by_protein_coding_autosomal_and_X_mapping",
    "protein_coding_genes_removed_outside_autosomal_and_X_chromosomes",
    "genes_before_filterByExpr",
  ...
  value = c(
    ...
    genes_in_raw_matrix,
    genes_in_complete_local_protein_coding_annotation,
    genes_in_local_autosomal_and_X_annotation,
    genes_after_mapping,
    genes_removed_by_mapping,
    protein_coding_genes_removed_outside_autosomal_and_X,
    genes_before_filterByExpr,
    genes_after_filterByExpr,
    genes_removed_by_filterByExpr,
    tested_genes_n,
...
output_files <- list(
  ...
  mapping_status = file.path(
    output_dir,
    "sexInteraction_proteinCodingAutosomalAndXMappingStatus.tsv"
  ),
...
readme_lines <- c(
  ...
  "Protein-coding genes restricted to autosomes 1-19 and chromosome X",
  ...
  paste0(
    "Genes in complete local Ensembl 115 protein-coding annotation: ",
    format_integer_for_readme(
      genes_in_complete_local_protein_coding_annotation
    )
  ),
  paste0(
    "Genes in autosomal-and-X analysis annotation: ",
    format_integer_for_readme(
      genes_in_local_autosomal_and_X_annotation
    )
  ),
  paste0(
    "Genes retained after autosomal-and-X protein-coding mapping: ",
    format_integer_for_readme(
      genes_after_mapping
    )
  ),
  paste0(
    "Genes removed during autosomal-and-X protein-coding mapping: ",
    format_integer_for_readme(
      genes_removed_by_mapping
    )
  ),
  ...
  paste0(
    "Protein-coding autosomal-and-X mapping status: ",
    basename(
      output_files$mapping_status
    )
  ),
...
message(
  "\nGene set: protein-coding genes on chromosomes 1-19 and X"
)
...
cat(
  sprintf(
    paste0(
      "The initial pseudobulk count matrix contained %s genes, ",
      "of which %s were protein-coding genes on autosomes 1-19 or chromosome X. ",
