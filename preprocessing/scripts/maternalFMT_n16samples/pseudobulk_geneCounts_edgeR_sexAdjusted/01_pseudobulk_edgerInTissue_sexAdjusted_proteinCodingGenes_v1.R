...
#   7. Apply edgeR::filterByExpr using the sex-adjusted design.
...
#  10. Save raw pseudobulk counts for tested genes.
#  11. Save TMM log2CPM followed by quantile normalization for visualization.
#  12. Save a simple two-column gene/filter-stage table.
#
...
#   maternalFMT_n16samples/pseudobulk_geneCounts_edgeR_sexAdjusted
# ==============================================================================
...
# ==============================================================================
# 18. Prepare the sex-adjusted edgeR design
#
# Model:
#   expression ~ fmt_donor_group + sex
#
# This tests one overall ASD vs Neurotypical effect after accounting for the
# additive effect of sex. No interaction term is included.
# ==============================================================================

sexAdjusted_metadata <- sample_metadata_analysis |>
  dplyr::select(
    sample_ID,
    fmt_donor_group,
    sex
...
if (!identical(
  as.character(sexAdjusted_metadata$sample_ID),
  colnames(gene_counts_per_sample_raw_in_tissue)
)) {
  stop("Metadata order does not match the count-matrix column order.")
}

if (anyNA(sexAdjusted_metadata$fmt_donor_group)) {
  stop("Missing or unexpected fmt_donor_group values.")
}

if (anyNA(sexAdjusted_metadata$sex)) {
  stop("Missing or unexpected sex values.")
}

group_counts <- table(
  sexAdjusted_metadata$fmt_donor_group
)

sex_group_counts <- table(
  sexAdjusted_metadata$sex,
  sexAdjusted_metadata$fmt_donor_group
)
...
sexAdjusted_design <- model.matrix(
  ~ fmt_donor_group + sex,
  data = sexAdjusted_metadata
)

rownames(sexAdjusted_design) <- as.character(
  sexAdjusted_metadata$sample_ID
)
...
if (!identical(
  colnames(sexAdjusted_design),
  expected_design_columns
)) {
  stop(
    "Unexpected sex-adjusted design columns: ",
    paste(
      colnames(sexAdjusted_design),
      collapse = ", "
    )
...
if (qr(sexAdjusted_design)$rank != ncol(sexAdjusted_design)) {
  stop("The sex-adjusted design matrix is not full rank.")
}

group_effect_coefficient <- which(
  colnames(sexAdjusted_design) ==
    "fmt_donor_groupASD"
)

if (length(group_effect_coefficient) != 1L) {
  stop(
    "Could not uniquely identify the ",
    "fmt_donor_groupASD coefficient."
  )
}

message("\nSex-adjusted design matrix:")
print(sexAdjusted_design)
...
sexAdjusted_dge_unfiltered <- edgeR::DGEList(
...
sexAdjusted_cpm_unfiltered <- edgeR::cpm(
  sexAdjusted_dge_unfiltered,
  log = FALSE
)

sexAdjusted_keep_genes <- edgeR::filterByExpr(
  sexAdjusted_dge_unfiltered,
  design = sexAdjusted_design,
...
if (
  !is.logical(sexAdjusted_keep_genes) ||
  length(sexAdjusted_keep_genes) !=
    nrow(sexAdjusted_dge_unfiltered$counts)
) {
  stop("Unexpected filterByExpr output.")
}

if (!any(sexAdjusted_keep_genes)) {
  stop("No genes passed filterByExpr.")
}

filterByExpr_status <- tibble::tibble(
  ensembl_gene_id = rownames(
    sexAdjusted_dge_unfiltered$counts
  ),
  total_count = rowSums(
    sexAdjusted_dge_unfiltered$counts
  ),
  ...
  samples_with_nonzero_count = rowSums(
    sexAdjusted_dge_unfiltered$counts > 0
  ),
  samples_with_count_ge_10 = rowSums(
    sexAdjusted_dge_unfiltered$counts >= 10
  ),
  mean_CPM = rowMeans(
    sexAdjusted_cpm_unfiltered
  ),
  max_CPM = apply(
    sexAdjusted_cpm_unfiltered,
    1,
  ...
  passed_filterByExpr = sexAdjusted_keep_genes,
  filterByExpr_status = if_else(
    sexAdjusted_keep_genes,
    "passed_filterByExpr",
...
sexAdjusted_dge <- sexAdjusted_dge_unfiltered[
  sexAdjusted_keep_genes,
  ,
  keep.lib.sizes = FALSE
]

sexAdjusted_dge <- edgeR::calcNormFactors(
  sexAdjusted_dge,
  method = "TMM"
)
...
sexAdjusted_dge <- edgeR::estimateDisp(
  sexAdjusted_dge,
  design = sexAdjusted_design,
  robust = TRUE
)

sexAdjusted_fit <- edgeR::glmQLFit(
  sexAdjusted_dge,
  design = sexAdjusted_design,
  robust = TRUE
)

sexAdjusted_test <- edgeR::glmQLFTest(
  sexAdjusted_fit,
  coef = group_effect_coefficient
)
...
sexAdjusted_fullResults <- edgeR::topTags(
  sexAdjusted_test,
  n = Inf,
  sort.by = "PValue"
)$table |>
...
# ==============================================================================
# 22. Create raw and normalized expression matrices
#
# RAW COUNTS
#   Exact integer pseudobulk counts for genes retained by filterByExpr and tested
#   by edgeR.
#
# NORMALIZED MATRIX
#   1. edgeR TMM normalization;
#   2. log2 CPM transformation with prior.count = 2;
#   3. quantile normalization across samples with limma.
#
# IMPORTANT
#   The quantile-normalized matrix is saved for visualization, clustering and
#   dimensional-reduction purposes only. The edgeR differential-expression model
#   is fitted to raw counts with TMM normalization, not to the QN matrix.
# ==============================================================================

sexAdjusted_rawCounts_testedGenes <-
  sexAdjusted_dge$counts

sexAdjusted_TMM_log2CPM <- edgeR::cpm(
  sexAdjusted_dge,
  log = TRUE,
  prior.count = 2,
...
sexAdjusted_TMM_log2CPM_QN <-
  limma::normalizeBetweenArrays(
    sexAdjusted_TMM_log2CPM,
    method = "quantile"
  )

rownames(sexAdjusted_TMM_log2CPM_QN) <-
  rownames(sexAdjusted_TMM_log2CPM)

colnames(sexAdjusted_TMM_log2CPM_QN) <-
  colnames(sexAdjusted_TMM_log2CPM)
...
matrix_to_gene_table <- function(
  ...
  matched_annotation <- gene_annotation[
    match(
  ...
  if (anyNA(matched_annotation$ensembl_gene_id)) {
    stop(
      "At least one expression-matrix gene was not found ",
      "in the local annotation."
    )
  }

  expression_data_frame <- as.data.frame(
    expression_matrix,
    check.names = FALSE,
    stringsAsFactors = FALSE
  )
  ...
  tibble::tibble(
    ...
    bind_cols(
      tibble::as_tibble(
        expression_data_frame,
        .name_repair = "minimal"
      )
    )
}

sexAdjusted_rawCounts_output <- matrix_to_gene_table(
  sexAdjusted_rawCounts_testedGenes,
  protein_coding_gene_annotation
)

sexAdjusted_normalizedCounts_output <-
...
# ==============================================================================
# 23. Create complete mapping and filtering status tables
# ==============================================================================
...
all_gene_filter_status <- protein_coding_mapping_status |>
  ...
  mutate(
    ...
    final_filter_status = case_when(
      !passed_protein_coding_filter ~
        "removed_not_in_local_Ensembl115_protein_coding_annotation",
      passed_protein_coding_filter &
        !passed_filterByExpr ~
...
gene_filtering_stage <- all_gene_filter_status |>
  transmute(
    gene = gene,
    filter_stage = final_filter_status
  ) |>
  arrange(
    filter_stage,
    gene
...
# ==============================================================================
# 24. Create filtering and analysis summaries
# ==============================================================================
...
significant_FDR_0.05 <- sum(
  sexAdjusted_fullResults$FDR <= 0.05,
  na.rm = TRUE
)

significant_FDR_0.05_logFC_0.5 <- sum(
  sexAdjusted_fullResults$FDR <= 0.05 &
    abs(sexAdjusted_fullResults$logFC) > 0.5,
  na.rm = TRUE
)
...
filtering_summary <- tibble::tibble(
  ...
  value = c(
    ...
    nrow(sexAdjusted_metadata),
    unname(
      sex_group_counts["Male", "Neurotypical"]
    ),
    unname(
      sex_group_counts["Male", "ASD"]
    ),
    unname(
      sex_group_counts["Female", "Neurotypical"]
    ),
    unname(
      sex_group_counts["Female", "ASD"]
    ),
    genes_in_raw_matrix,
    genes_in_local_annotation,
    genes_after_protein_coding_mapping,
...
analysis_summary <- tibble::tibble(
  ...
  sex_reference_level = "Male",
  interaction_included = FALSE,
  n_samples = nrow(sexAdjusted_metadata),
...
# ==============================================================================
# 25. Prepare small supporting output tables
# ==============================================================================

sexAdjusted_design_output <- data.frame(
  sample_ID = rownames(
    sexAdjusted_design
  ),
  sexAdjusted_design,
...
sexAdjusted_sample_metadata_output <-
  sexAdjusted_metadata |>
  mutate(
    fmt_donor_group = as.character(
      fmt_donor_group
    ),
...
output_files <- list(
  ...
  raw_counts_tested_genes = file.path(
    output_dir,
    "sexAdjusted_rawCounts_testedGenes.tsv"
  ),
  normalized_counts_tested_genes = file.path(
    output_dir,
    paste0(
      "sexAdjusted_TMM_log2CPM_",
      "quantileNormalized_testedGenes.tsv"
    )
  ),
  gene_filtering_stage = file.path(
    output_dir,
    "sexAdjusted_geneFilteringStage.tsv"
  ),
  analysis_summary = file.path(
    output_dir,
    "sexAdjusted_analysisSummary.tsv"
  ),
  filtering_summary = file.path(
    output_dir,
    "sexAdjusted_filteringSummary.tsv"
  ),
  all_gene_filter_status = file.path(
    output_dir,
    "sexAdjusted_allGeneFilterStatus.tsv"
  ),
  protein_coding_mapping_status = file.path(
    output_dir,
    "sexAdjusted_proteinCodingMappingStatus.tsv"
  ),
  filterByExpr_status = file.path(
    output_dir,
    "sexAdjusted_filterByExprStatus.tsv"
  ),
  sample_inclusion_status = file.path(
    output_dir,
    "sexAdjusted_sampleInclusionStatus.tsv"
  ),
  pseudobulk_input_summary = file.path(
    output_dir,
    "sexAdjusted_pseudobulkInputSummary.tsv"
  ),
  ...
  design_matrix = file.path(
    output_dir,
    "sexAdjusted_designMatrix.tsv"
  ),
  reference_annotation_info = file.path(
    output_dir,
    "sexAdjusted_referenceAnnotationInfo.tsv"
  )
)


# ==============================================================================
# 27. Save all outputs as TSV files
# ==============================================================================
...
write_tsv_base(
  sexAdjusted_rawCounts_output,
  output_files$raw_counts_tested_genes
)

write_tsv_base(
  sexAdjusted_normalizedCounts_output,
  output_files$normalized_counts_tested_genes
)

write_tsv_base(
  gene_filtering_stage,
  output_files$gene_filtering_stage
)
...
# ==============================================================================
# 28. Validate saved files and tables
# ==============================================================================
...
if (!identical(
  ...
  stop(
    "Unexpected full-result columns: ",
    paste(
      colnames(sexAdjusted_fullResults),
      collapse = ", "
    )
...
if (!identical(
  colnames(gene_filtering_stage),
  c("gene", "filter_stage")
)) {
  stop(
    "gene_filtering_stage must contain exactly: ",
    "gene, filter_stage"
  )
}

if (!identical(
  colnames(sexAdjusted_rawCounts_output),
  colnames(sexAdjusted_normalizedCounts_output)
)) {
  stop(
    "Raw and normalized count-table columns do not match."
  )
}

if (!identical(
  sexAdjusted_rawCounts_output$ensembl_gene_id,
  sexAdjusted_normalizedCounts_output$ensembl_gene_id
)) {
  stop(
    "Raw and normalized count-table gene orders do not match."
  )
}
...
message("SEX-ADJUSTED edgeR ANALYSIS COMPLETED SUCCESSFULLY")
...
message(
  "Comparison: ASD vs Neurotypical adjusted for sex"
)
message(
  "Samples used: ",
  nrow(sexAdjusted_metadata)
...
message("\nExpression matrices:")
message(
  "  Raw counts: tested genes only; integer pseudobulk counts."
)
message(
  paste0(
    "  Normalized: tested genes only; ",
    "TMM log2CPM followed by quantile normalization."
  )
)
message(
  paste0(
    "  The quantile-normalized matrix is not used ",
    "for edgeR differential-expression testing."
  )
)
...
message("  sexAdjusted_fullResults")
