...
# ==============================================================================
# 01_functionalAnalysis_effectSex_multiEnrichrLibraries_FIXED_v02.R
#
# Functional enrichment analysis for the main sex effect from the
...
#
# INTERPRETATION
#   UP   = positive logFC = higher expression in Female
#   DOWN = negative logFC = higher expression in Male
#
# ENRICHR LIBRARIES
#   ChEA_2022
...
#
# PER-DATABASE WORK
#   3 direction variants x 17 scopes = 51 Enrichr lists.
...
# Exact sex-effect selection used by the sex heatmap.
sex_test_id <- "Overall_Sex_Female_vs_Male"
sex_fdr_threshold <- 0.05
sex_abs_log2fc_threshold <- 0.5
...
analysis_variants <- data.frame(
  xlsx_index = 1:3,
  subset_id = analysis_directions,
  subset_label = c(
    "all significant sex-effect genes",
    "Female > Male",
    "Male > Female"
  ),
  file_suffix = c(
    "ALL_enrichrResults.xlsx",
    "UP_enrichrResults.xlsx",
    "DOWN_enrichrResults.xlsx"
  ),
  stringsAsFactors = FALSE
)
...
# ==============================================================================
# 5. Helper: no test-searching is required for the sex main effect
# ==============================================================================
#
# The sex-effect test is fixed explicitly in the configuration:
#
#   Overall_Sex_Female_vs_Male
#
# The functional-analysis input is therefore read directly from:
#
#   edgeR_perCluster_combinedResults[[sex_test_id]]
#
# edgeR_perCluster_testDefinitions is used only to retrieve optional descriptive
# metadata (comparison/sheet name). Failure to match that metadata does NOT stop
# the analysis.
#
# ==============================================================================
...
write_direction_workbook <- function(
  direction,
  result_list,
  output_file
) {
...
# ==============================================================================
# 13. Select the sex main-effect results directly and select significant genes
# ==============================================================================

message(
  "\nRequested sex test_id: ",
  sex_test_id
)

available_result_tests <- names(
  edgeR_perCluster_combinedResults
)

if (is.null(available_result_tests)) {
  stop(
    "edgeR_perCluster_combinedResults has no names; cannot select the sex test.",
    call. = FALSE
  )
}

if (!sex_test_id %in% available_result_tests) {
  message(
    "\nAvailable tests in edgeR_perCluster_combinedResults:"
  )
  message(
    paste(
      available_result_tests,
      collapse = "\n"
    )
  )

  stop(
    "Sex main-effect test is absent from edgeR_perCluster_combinedResults: ",
    sex_test_id,
    call. = FALSE
  )
}
...
# IMPORTANT:
# This is the actual analysis input. No dplyr filtering or heuristic matching
# is used to choose the sex-effect result table.
sex_results <- edgeR_perCluster_combinedResults[[sex_test_id]] |>
  tibble::as_tibble()

message(
  "Selected sex result table directly from edgeR_perCluster_combinedResults[[\"",
  sex_test_id,
  "\"]]."
)

# Retrieve descriptive metadata only if an exact testDefinitions match exists.
# This metadata is informational and can never prevent the enrichment analysis.
test_definition_ids <- trimws(
  as.character(
    edgeR_perCluster_testDefinitions$test_id
  )
)

sex_definition_index <- match(
  sex_test_id,
  test_definition_ids
)

if (is.na(sex_definition_index)) {
  selected_test_comparison <- "Female vs Male"
  selected_test_sheet <- "Overall_Sex"

  warning(
    "Could not match the sex test in edgeR_perCluster_testDefinitions. ",
    "Continuing because the actual result table was found directly in ",
    "edgeR_perCluster_combinedResults."
  )
} else {
  selected_test_comparison <- as.character(
    edgeR_perCluster_testDefinitions$comparison[[sex_definition_index]]
  )

  selected_test_sheet <- as.character(
    edgeR_perCluster_testDefinitions$sheet_name[[sex_definition_index]]
  )
}
...
missing_sex_columns <- setdiff(
  required_sex_columns,
  colnames(sex_results)
)

if (length(missing_sex_columns) > 0L) {
  stop(
    "Sex-effect result table is missing required column(s): ",
    paste(
      missing_sex_columns,
      collapse = ", "
    ),
...
if (length(unexpected_cluster_ids) > 0L) {
  warning(
    "Unexpected cluster IDs in sex-effect results: ",
    paste(
      sort(
        unexpected_cluster_ids
      ),
      collapse = ", "
    )
  )
...
# Exact gene selection from the sex-effect heatmap:
#   FDR < 0.05
#   abs(log2FC) >= 0.5
#
# For Female vs Male:
#   UP   = logFC > 0 = Female > Male
#   DOWN = logFC < 0 = Male > Female
significant_sex_results <- sex_results |>
  dplyr::filter(
    !is.na(.data$FDR),
    !is.na(.data$logFC),
    .data$FDR < sex_fdr_threshold,
    abs(.data$logFC) >= sex_abs_log2fc_threshold
  ) |>
  dplyr::mutate(
...
if (nrow(significant_sex_results) == 0L) {
  stop(
    "No sex-effect result satisfies FDR < ",
    sex_fdr_threshold,
    " and abs(log2FC) >= ",
    sex_abs_log2fc_threshold,
    ".",
    call. = FALSE
  )
}
...
message(
  "  UP cluster-gene results = ",
  sum(
    significant_sex_results$effect_direction == "UP",
    na.rm = TRUE
  )
)

message(
  "  DOWN cluster-gene results = ",
  sum(
    significant_sex_results$effect_direction == "DOWN",
    na.rm = TRUE
  )
)
...
message(
  ...
  sum(
    is.na(
      significant_sex_results$gene_symbol
    )
  )
)
...
apply_direction_filter <- function(
  data,
  direction
) {
  if (direction == "ALL") {
...
for (variant_index in seq_len(nrow(analysis_variants))) {
  direction <- analysis_variants$subset_id[[variant_index]]
  ...
  query_by_variant[[direction]] <- list()

  # Global list = union of unique gene symbols across all significant clusters
  # after applying the requested direction filter.
  #
  # UP and DOWN are defined by the Female-vs-Male contrast:
  #   UP   = logFC > 0 = Female > Male
  #   DOWN = logFC < 0 = Male > Female
  #
  # A gene may legitimately occur in both global UP and DOWN workbooks if it
  # has opposite sex-effect directions in different anatomical clusters.
  query_by_variant[[direction]][["allGenes"]] <- clean_gene_vector(
    direction_data$gene_symbol
  )
  ...
  for (cluster_id_current in expected_cluster_ids) {
    ...
    query_by_variant[[direction]][[cluster_id_current]] <- clean_gene_vector(
...
for (variant_index in seq_len(nrow(analysis_variants))) {
  ...
  message(
    ...
    " | global = ",
    length(
      query_by_variant[[direction]][["allGenes"]]
    )
  )
}

message(
  "  NOTE: UP = Female > Male; DOWN = Male > Female."
)
...
message(
  "MULTI-DATABASE ENRICHR RUN: SEX MAIN EFFECT"
)
...
message(
  "MULTI-DATABASE SEX-EFFECT FUNCTIONAL ANALYSIS FINISHED"
)
...
message(
  "UP = Female > Male | DOWN = Male > Female"
)
