...
# ==============================================================================
# Cluster-to-cluster correlation heatmaps for the FMT donor-group main effect
#
...
find_donor_group_test_id <- function(test_definitions) {
  ...
  as.character(best_rows$test_id[[1]])
}
...
# ==============================================================================
# 7. Select FMT donor-group main-effect test
# ==============================================================================
...
# ==============================================================================
# 8. Prepare cluster-specific log2FC results and gene sets
# ==============================================================================
...
cluster_ids <- sort_cluster_ids(
  donor_group_results$cluster_id
)
...
all_gene_ids <- donor_group_results |>
  dplyr::filter(
...
significant_cluster_gene_results <- donor_group_results |>
  dplyr::filter(
    !is.na(.data$FDR),
    is.finite(.data$logFC),
...
if (length(all_gene_ids) == 0L) {
  stop(
    "No finite donor-group logFC values were found.",
    call. = FALSE
  )
}
...
for (variant_index in seq_len(nrow(correlation_variants))) {
  ...
  logfc_matrix <- build_logfc_matrix(
    results_table = donor_group_results,
  ...
  heatmap_title_lines <- c(
    paste0(
      "FMT donor-group main effect: cluster-to-cluster correlation of log2FC profiles (",
