...
# ==============================================================================
# Cluster-to-cluster correlation heatmaps for the Group x Sex interaction
#
# TEST
#   Interaction test identified from edgeR_perCluster_testDefinitions
#
# INTERPRETATION
#   Correlations are calculated from the cluster-specific interaction log2FC.
#   Positive and negative interaction log2FC values are retained as signed effects;
#   no UP/DOWN subdivision is used.
#
...
# ==============================================================================
# 7. Select Group x Sex interaction test
# ==============================================================================
...
interaction_test_definition <- edgeR_perCluster_testDefinitions |>
  dplyr::filter(.data$test_id == interaction_test_id)

if (nrow(interaction_test_definition) != 1L) {
  stop(
    "Interaction test_id does not map to exactly one test definition: ",
    interaction_test_id,
    call. = FALSE
  )
}
...
selected_test_sheet <- as.character(
  interaction_test_definition$sheet_name[[1]]
)
...
# ==============================================================================
# 8. Prepare cluster-specific interaction log2FC results and gene sets
# ==============================================================================
...
all_gene_ids <- interaction_results |>
  dplyr::filter(
...
significant_cluster_gene_results <- interaction_results |>
  dplyr::filter(
    !is.na(.data$FDR),
    is.finite(.data$logFC),
...
if (length(all_gene_ids) == 0L) {
  stop(
    "No finite interaction logFC values were found.",
    call. = FALSE
  )
}
...
for (variant_index in seq_len(nrow(correlation_variants))) {
  ...
  if (identical(gene_set, "all")) {
    ...
    gene_set_definition <- paste0(
      ...
      fdr_threshold,
      "; for these genes, interaction log2FC is then taken from every cluster in which the gene was tested"
    )

    output_gene_set_id <- paste0(
      "significantUnion_FDR005"
    )
  ...
  heatmap_title_lines <- c(
    paste0(
      "Group x Sex interaction: cluster-to-cluster correlation of interaction log2FC profiles (",
    ...
    "Effect values: signed Group x Sex interaction log2FC | no UP/DOWN subdivision",
    ...
    paste0(
      "Correlation input: cluster-specific edgeR interaction log2FC",
  ...
  analysis_info <- tibble::tibble(
    parameter = c(
      ...
      "test_id",
      "comparison",
      "interaction_log2FC_interpretation",
      "test_sheet",
      "gene_set",
      "gene_set_definition",
      "selection_note",
    ...
    value = c(
      ...
      interaction_test_id,
      selected_test_comparison,
      "signed interaction coefficient; positive/negative values retained without UP/DOWN subdivision",
      selected_test_sheet,
      gene_set_label,
      gene_set_definition,
      selection_note,
