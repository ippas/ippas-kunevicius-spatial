...
# ==============================================================================
# 05_geneExpression_spatialMaps_groupBarplot_v2.R
#
# Purpose:
...
#     * percentage of target-gene-positive tissue spots
#     * mean +/- SD of log-normalized expression across ALL tissue spots,
#       including spots with zero target-gene counts
# - save results into two clean subdirectories:
#     1. pdf/     - combined spatial-map PDF and group barplot PDF
#     2. summary/ - sample-level and group-level TSV summaries
# - create a four-group barplot using sample-level means as observations:
#     * empty bars = group means
#     * dots = individual sample means
#     * whiskers = mean +/- SE across samples
#
# Notes:
...
barplot_pdf_width_inches <- 8.5
barplot_pdf_height_inches <- 6.5
...
# Strong red palette:
# - exact/near-zero values remain grey
# - positive values move quickly into pink/red tones
# - non-linear colour positions reduce the visually blurred yellow/orange range
palette_colors <- c(
  "#D9D9D9",
  "#FEE5D9",
  "#FCAE91",
...
for (directory in c(output_dir, summary_dir, pdf_dir)) {
  dir.create(
    directory,
    recursive = TRUE,
    showWarnings = FALSE
...
output_barplot_pdf <- file.path(
  ...
  paste0(
    "05_",
    target_gene,
    "_sampleMean_groupBarplot_meanSE.pdf"
  )
)
...
# ==============================================================================
# 12. Create and save the four-group barplot and spatial-map PDF
# ==============================================================================
...
group_barplot <- ggplot() +
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      "\nEach sample mean includes all tissue spots, including zero-count spots"
    ),
    x = NULL,
    y = paste0(
      "Mean ",
