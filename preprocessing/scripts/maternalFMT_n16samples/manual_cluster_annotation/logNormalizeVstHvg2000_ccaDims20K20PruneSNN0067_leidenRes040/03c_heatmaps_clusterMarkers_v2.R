...
#
# Purpose:
# Create one globally clustered heatmap for marker genes with tau >= 0.80, while promoting proximity of rows and columns from the same cluster.
#
# ROWS
...
row_same_cluster_bonus <- 0.40
column_same_cluster_bonus <- 0.40
...
output_directory <- file.path(
  ...
  "manual_cluster_annotation",
  configuration_name,
  "03_geneSpecificityHeatmap_tau080_clusterWeighted_noSplits"
)
...
heatmap_pdf <- file.path(
  ...
  paste0(
    ...
    "sampleByClusterMeanExpression_rowZscore_clusterWeighted_noSplits_heatmap.pdf"
  )
)
...
ordered_rows_tsv <- file.path(
  output_directory,
  paste0(
    "08_",
    dataset_name,
    "_leidenRes040_",
    "globallyClustered_rowOrder.tsv"
  )
)

ordered_columns_tsv <- file.path(
  output_directory,
  paste0(
    "09_",
    dataset_name,
    "_leidenRes040_",
    "globallyClustered_columnOrder.tsv"
  )
)

row_clustering_contingency_tsv <- file.path(
  output_directory,
  paste0(
    "10_",
    dataset_name,
    "_leidenRes040_",
    "rowHierarchicalGroupsK16_vsMarkerClusters.tsv"
  )
)

column_clustering_contingency_tsv <- file.path(
  output_directory,
  paste0(
    "11_",
    dataset_name,
    "_leidenRes040_",
    "columnHierarchicalGroupsK16_vsOriginalClusters.tsv"
  )
)
...
# ==============================================================================
# 12. Prepare row and column annotations without imposing hard splits
# ==============================================================================
...
row_marker_cluster_annotation <- factor(
  selected_gene_table$marker_cluster,
  levels = cluster_ids
)

column_cluster_annotation <- factor(
...
if (anyNA(row_marker_cluster_annotation)) {
  stop(
    "Unexpected representative marker-cluster value.",
    call. = FALSE
  )
}

if (anyNA(column_cluster_annotation)) {
  stop(
    "Unexpected source-cluster value in column metadata.",
    call. = FALSE
  )
}
...
# The row colour bar indicates the representative marker cluster assigned to
# each gene. Preference is given to assignments in which the gene reaches its
# highest mean expression in that cluster (is_best_cluster = TRUE).
right_annotation <- ComplexHeatmap::rowAnnotation(
  `Marker cluster` = row_marker_cluster_annotation,
  col = list(
    `Marker cluster` = custom_cluster_colors[cluster_ids]
  ),
...
top_annotation <- ComplexHeatmap::HeatmapAnnotation(
  Cluster = column_cluster_annotation,
...
heatmap_title <- paste(
  c(
    "Global hierarchical clustering of cluster-specific marker genes and sample-by-cluster profiles",
    paste0(
      number_of_selected_unique_genes,
      " unique genes | ",
      number_of_selected_marker_assignments,
    ...
    paste0(
      "Global row and column clustering: Pearson correlation distance",
      " | complete linkage | no predefined row or column splits"
    ),
    paste0(
      "Same-cluster distance weighting: rows bonus = ",
      formatC(row_same_cluster_bonus, format = "f", digits = 2),
      ", columns bonus = ",
      formatC(column_same_cluster_bonus, format = "f", digits = 2),
      " | annotation bar at right shows representative marker cluster"
    )
  ),
  collapse = "\n"
)
...
# ==============================================================================
# 15. Global hierarchical clustering of all rows and all columns
# ==============================================================================
...
# Global clustering is retained, but distances are mildly modified so that rows
# or columns associated with the same cluster are promoted to stay closer
# together. This does not impose any hard split.
row_correlation_matrix <- suppressWarnings(
  stats::cor(
    t(zscore_matrix),
    method = "pearson",
...
row_correlation_matrix[
  !is.finite(row_correlation_matrix)
] <- 0

diag(row_correlation_matrix) <- 1

row_distance_matrix <- 1 - row_correlation_matrix

row_same_cluster_matrix <- outer(
  as.character(selected_gene_table$marker_cluster),
  as.character(selected_gene_table$marker_cluster),
  FUN = "=="
)

row_distance_matrix[row_same_cluster_matrix] <-
  row_distance_matrix[row_same_cluster_matrix] *
  (1 - row_same_cluster_bonus)

row_distance_matrix[!row_same_cluster_matrix] <-
  row_distance_matrix[!row_same_cluster_matrix] *
  (1 + row_same_cluster_bonus)

diag(row_distance_matrix) <- 0
row_distance_matrix[row_distance_matrix < 0] <- 0

row_hclust <- stats::hclust(
  stats::as.dist(row_distance_matrix),
  method = "complete"
)

column_correlation_matrix <- suppressWarnings(
  stats::cor(
    zscore_matrix,
    method = "pearson",
    use = "pairwise.complete.obs"
...
column_correlation_matrix[
  !is.finite(column_correlation_matrix)
] <- 0

diag(column_correlation_matrix) <- 1

column_distance_matrix <- 1 - column_correlation_matrix

column_same_cluster_matrix <- outer(
  as.character(column_metadata$cluster_id),
  as.character(column_metadata$cluster_id),
  FUN = "=="
)

column_distance_matrix[column_same_cluster_matrix] <-
  column_distance_matrix[column_same_cluster_matrix] *
  (1 - column_same_cluster_bonus)

column_distance_matrix[!column_same_cluster_matrix] <-
  column_distance_matrix[!column_same_cluster_matrix] *
  (1 + column_same_cluster_bonus)

diag(column_distance_matrix) <- 0
column_distance_matrix[column_distance_matrix < 0] <- 0

column_hclust <- stats::hclust(
  stats::as.dist(column_distance_matrix),
  method = "complete"
)

# Cut both dendrograms into 16 groups to quantify whether the unconstrained
# hierarchical clustering still recovers the predefined clusters.
row_hierarchical_group_k16 <- stats::cutree(
  row_hclust,
  k = expected_number_of_clusters
)

column_hierarchical_group_k16 <- stats::cutree(
  column_hclust,
  k = expected_number_of_clusters
)

row_contingency_table <- table(
  Marker_cluster = selected_gene_table$marker_cluster,
  Hierarchical_group_k16 = row_hierarchical_group_k16
)

column_contingency_table <- table(
  Original_cluster = column_metadata$cluster_id,
  Hierarchical_group_k16 = column_hierarchical_group_k16
)

row_cluster_recovery_purity <-
  sum(
    apply(
      row_contingency_table,
      MARGIN = 2,
      FUN = max
    )
  ) /
...
column_cluster_recovery_purity <-
  sum(
    apply(
      column_contingency_table,
      MARGIN = 2,
      FUN = max
    )
  ) /
...
row_sum_choose_two <- sum(
  choose(
    rowSums(row_contingency_table),
    2
  )
)

row_column_sum_choose_two <- sum(
  choose(
    colSums(row_contingency_table),
    2
  )
)

row_cell_sum_choose_two <- sum(
  choose(
    row_contingency_table,
    2
  )
...
row_total_pairs <- choose(
  sum(row_contingency_table),
  2
)

row_expected_index <-
  row_sum_choose_two *
  row_column_sum_choose_two /
  row_total_pairs

row_maximum_index <- 0.5 * (
  row_sum_choose_two +
    row_column_sum_choose_two
)

row_adjusted_rand_index <- if (
  row_maximum_index == row_expected_index
) {
  NA_real_
} else {
  (
    row_cell_sum_choose_two -
      row_expected_index
  ) /
    (
      row_maximum_index -
...
column_row_sum_choose_two <- sum(
  choose(
    rowSums(column_contingency_table),
    2
  )
)

column_column_sum_choose_two <- sum(
  choose(
    colSums(column_contingency_table),
    2
  )
)

column_cell_sum_choose_two <- sum(
  choose(
    column_contingency_table,
    2
  )
...
column_total_pairs <- choose(
  sum(column_contingency_table),
  2
)

column_expected_index <-
  column_row_sum_choose_two *
  column_column_sum_choose_two /
  column_total_pairs

column_maximum_index <- 0.5 * (
  column_row_sum_choose_two +
    column_column_sum_choose_two
)

column_adjusted_rand_index <- if (
  column_maximum_index == column_expected_index
) {
  NA_real_
} else {
  (
    column_cell_sum_choose_two -
      column_expected_index
  ) /
    (
...
message(
  "\nRow-cluster recovery after cutting the global dendrogram at k = 16:"
)
...
message(
  "\nColumn-cluster recovery after cutting the global dendrogram at k = 16:"
)
...
heatmap_object <- ComplexHeatmap::Heatmap(
  ...
  cluster_columns = column_hclust,
  column_dend_reorder = TRUE,
  column_dend_height = grid::unit(
    18,
    "mm"
  ),
...
row_order_indices <- row_hclust$order
column_order_indices <- column_hclust$order

ordered_row_output <- selected_gene_output[
  row_order_indices,
  ,
  drop = FALSE
]

ordered_row_output$heatmap_row_order <- seq_len(
  nrow(ordered_row_output)
)

ordered_row_output$hierarchical_group_k16 <-
  row_hierarchical_group_k16[
    row_order_indices
  ]

ordered_row_output <- ordered_row_output[
  ,
  c(
    "heatmap_row_order",
    "hierarchical_group_k16",
    setdiff(
      colnames(ordered_row_output),
      c(
        "heatmap_row_order",
        "hierarchical_group_k16"
      )
...
ordered_column_output <- column_metadata[
  column_order_indices,
  ,
  drop = FALSE
]

ordered_column_output$heatmap_column_order <- seq_len(
  nrow(ordered_column_output)
)

ordered_column_output$hierarchical_group_k16 <-
  column_hierarchical_group_k16[
    column_order_indices
  ]

ordered_column_output <- ordered_column_output[
  ,
  c(
    "heatmap_column_order",
    "hierarchical_group_k16",
    setdiff(
      colnames(ordered_column_output),
      c(
        "heatmap_column_order",
        "hierarchical_group_k16"
      )
...
write.table(
  ordered_column_output,
  file = ordered_columns_tsv,
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
  col.names = TRUE,
...
row_contingency_output <- data.frame(
  Marker_cluster = rownames(row_contingency_table),
...
write.table(
  row_contingency_output,
  file = row_clustering_contingency_tsv,
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
  col.names = TRUE,
...
column_contingency_output <- data.frame(
  Original_cluster = rownames(column_contingency_table),
  as.data.frame.matrix(
    column_contingency_table,
    stringsAsFactors = FALSE
  ),
...
write.table(
  column_contingency_output,
  file = column_clustering_contingency_tsv,
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
  col.names = TRUE,
...
summary_table <- data.frame(
  ...
  row_clustering =
    "global Pearson correlation distance with complete linkage; same-marker-cluster pairs promoted",
  column_clustering =
    "global Pearson correlation distance with complete linkage; same-source-cluster pairs promoted",
  row_same_cluster_bonus = row_same_cluster_bonus,
  column_same_cluster_bonus = column_same_cluster_bonus,
  row_hierarchical_groups_k = expected_number_of_clusters,
  row_cluster_recovery_purity = row_cluster_recovery_purity,
  row_adjusted_rand_index = row_adjusted_rand_index,
  column_hierarchical_groups_k = expected_number_of_clusters,
  column_cluster_recovery_purity = column_cluster_recovery_purity,
  column_adjusted_rand_index = column_adjusted_rand_index,
...
expected_output_files <- c(
  ...
  ordered_rows_tsv,
  ordered_columns_tsv,
  row_clustering_contingency_tsv,
  column_clustering_contingency_tsv
)
...
message(
  "Column recovery at k = 16: purity = ",
  formatC(
    column_cluster_recovery_purity,
    format = "f",
    digits = 3
